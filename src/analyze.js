...
import { getItemsForCategory, getTrailingHistory, getDailyHistoryForItem, closeDb } from "./db.js";
...
async function main() {
  ...
  for (const cat of CATEGORIES) {
    ...
    for (const item of items) {
      ...
      const signal = computeSignal({ currentValue: current.primaryValue, history: baseline, sparkline: current.sparkline, trend });
  ...
  const confidenceRank = (r) => (r.confidence === "counterTrend" ? 1 : 0);
  const buys = liquid
    .filter((r) => r.verdict === "BUY")
    .sort((a, b) => confidenceRank(a) - confidenceRank(b) || a.totalChange - b.totalChange)
    .slice(0, 12);
  const sells = liquid
    .filter((r) => r.verdict === "SELL")
    .sort((a, b) => confidenceRank(a) - confidenceRank(b) || b.totalChange - a.totalChange)
    .slice(0, 12);

  console.log(`League: ${league.displayName}  |  liquid universe: ${liquid.length}/${rows.length} items  |  local samples/item: ${rows[0]?.n ?? 1}\n`);

  const fmt = (r) =>
    `  ${r.confidence === "counterTrend" ? "[COUNTER-TREND] " : ""}${r.category.padEnd(20)} ${r.name.padEnd(28)} ${r.value.toFixed(4)} ${r.currency}  liq=${r.listingCount ?? r.volume}  spark=[${r.sparkData.map((n) => n.toFixed(1)).join(", ")}]  total=${r.totalChange.toFixed(1)}%${r.trend && r.trend.regime !== "INSUFFICIENT" ? `  regime=${r.trend.regime}` : ""}`;
